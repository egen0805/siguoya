...
class ajax extends AWS_ADMIN_CONTROLLER{
	...
	public function approval_manage_action(){
		...
		if($_POST['approval_id']){
			$_POST['approval_ids']=[$_POST['approval_id']];
		}
				...
				switch($_POST['batch_type']){
					case 'approval':
						...
						foreach($_POST['approval_ids'] AS $approval_id){
							$this->model('openid_weibo_weibo')
								->save_msg_info_to_question($approval_id,$published_user['uid']);
						}
						break;
					case 'decline':
						foreach($_POST['approval_ids'] AS $approval_id){
			...
			case 'received_email':
				...
				switch($_POST['batch_type']){
					case 'approval':
						...
						foreach($_POST['approval_ids'] AS $approval_id){
							$this->model('edm')
								->save_received_email_to_question($approval_id,$receiving_email_global_config['publish_user']['uid']);
						}
						break;
					case 'decline':
						foreach($_POST['approval_ids'] AS $approval_id){
	...
	public function save_category_action(){
		...
		if($_POST['category_id'] AND $_POST['parent_id'] AND $category_list=$this->model('system')
				->fetch_category('question',$_POST['category_id'])
		){
		...
		$this->model('category')
			->update_category_info($category_id,$_POST['title'],$_POST['parent_id'],$_POST['url_token']);
		H::ajax_json_output(AWS_APP::RSM([
		...
		if($_FILES['icon']['name']){
			...
			if(AWS_APP::upload()->get_error()){
				switch(AWS_APP::upload()->get_error()){
					default:
						H::ajax_json_output(AWS_APP::RSM(null,-1,AWS_APP::lang()->_t('错误代码').': '.AWS_APP::upload()
								->get_error()));
						break;
		...
		if($_POST['nav_sort']){
			if($menu_ids=explode(',',$_POST['nav_sort'])){
				foreach($menu_ids as $key=>$val){
					$this->model('menu')->update_nav_menu($val,[
						'sort'=>$key
					]);
				}
			}
	...
	public function nav_menu_upload_action(){
		...
		$this->model('menu')->update_nav_menu($_GET['id'],['icon'=>basename($upload_data['full_path'])]);
	...
	public function add_page_action(){
		...
		$this->model('page')
			->add_page($_POST['title'],$_POST['keywords'],$_POST['description'],$_POST['contents'],$_POST['url_token']);
		H::ajax_json_output(AWS_APP::RSM([
	...
	public function edit_page_action(){
		...
		$this->model('page')
			->update_page($_POST['page_id'],$_POST['title'],$_POST['keywords'],$_POST['description'],$_POST['contents'],$_POST['url_token']);
		H::ajax_json_output(AWS_APP::RSM([
			'url'=>get_js_url('/admin/page/')
		],1,null));
	}
	...
	public function report_manage_action(){
		...
		if($_POST['action_type']=='delete'){
			...
			foreach($_POST['report_ids'] as $val){
				$this->model('question')->update_report($val,[
					'status'=>1
				]);
			}
		}
	...
	public function save_topic_action(){
		if($_POST['topic_id']){
			...
			if($topic_info['topic_title']!=$_POST['topic_title'] AND $this->model('topic')
					->get_topic_by_title($_POST['topic_title'])
			){
				H::ajax_json_output(AWS_APP::RSM(null,-1,AWS_APP::lang()->_t('同名话题已经存在')));
			}
			$this->model('topic')
				->update_topic($this->user_id,$topic_info['topic_id'],$_POST['topic_title'],$_POST['topic_description']);
			...
			$topic_id=$this->model('topic')
				->save_topic($_POST['topic_title'],$this->user_id,true,$_POST['topic_description']);
		}
		...
		if($group_new=$_POST['group_new']){
			foreach($group_new['group_name'] as $key=>$val){
				if(trim($group_new['group_name'][$key])){
					$this->model('account')
						->add_user_group($group_new['group_name'][$key],1,$group_new['reputation_lower'][$key],$group_new['reputation_higer'][$key],$group_new['reputation_factor'][$key]);
				}
			}
		}
	...
	public function edit_user_group_permission_action(){
		...
		$this->model('account')->update_user_group_data($_POST['group_id'],[
	...
	public function save_user_action(){
		if($_POST['uid']){
			...
			if($_POST['user_name']!=$user_info['user_name'] AND $this->model('account')
					->get_user_info_by_username($_POST['user_name'])
			){
				H::ajax_json_output(AWS_APP::RSM(null,-1,AWS_APP::lang()->_t('用户名已存在')));
			}
			if($_POST['email']!=$user_info['email'] AND $this->model('account')
					->get_user_info_by_username($_POST['email'])
			){
			...
			if($_FILES['user_avatar']['name']){
					...
					switch(AWS_APP::upload()->get_error()){
						...
						case 'upload_invalid_filesize':
							H::ajax_json_output(AWS_APP::RSM(null,-1,AWS_APP::lang()
								->_t('文件尺寸过大, 最大允许尺寸为 %s KB',get_setting('upload_size_limit'))));
							break;
					}
				}
				if(!$upload_data=AWS_APP::upload()->data()){
				...
				if($upload_data['is_image']==1){
					foreach(AWS_APP::config()->get('image')->avatar_thumbnail AS $key=>$val){
						$thumb_file[$key]=$upload_data['file_path'].$this->model('account')
								->get_avatar($user_info['uid'],$key,2);
						AWS_APP::image()->initialize([
							'quality'=>90,
							'source_image'=>$upload_data['full_path'],
							'new_image'=>$thumb_file[$key],
				...
				$update_data['avatar_file']=$this->model('account')
						->get_avatar($user_info['uid'],null,1).basename($thumb_file['min']);
			}
			if($_POST['email']){
			...
			if($verify_apply){
				...
				if(!$update_data['verified']){
					...
					$this->model('verify')
						->update_apply($user_info['uid'],null,null,null,null,$update_data['verified']);
				}
			...
			if($_POST['password']){
				$this->model('account')
					->update_user_password_ingore_oldpassword($_POST['password'],$user_info['uid'],fetch_salt(4));
			}
			$this->model('account')->update_users_attrib_fields([
			...
			if($_POST['group_id']!=4){
				$this->model('account')->update('users',[
	...
	public function send_invites_action(){
		...
		$this->model('invitation')
			->send_batch_invitations(array_unique($email_list),$this->user_id,$this->user_info['user_name']);
	...
	public function save_job_action(){
		if($_POST['job_list']){
			foreach($_POST['job_list'] as $key=>$val){
				$this->model('work')->update_job($key,[
					'job_name'=>$val,
				]);
			}
		}
			...
			case 'decline':
				foreach($_POST['approval_uids'] AS $approval_uid){
					if($user_info=$this->model('account')->get_user_info_by_uid($approval_uid)){
						if($user_info['email']){
							$this->model('email')->action_email('REGISTER_DECLINE',$user_info['email'],null,[
		...
		switch($_POST['action']){
			case 'add_service_user':
				if($service_info){
					...
					$tmp_service_account[$user_info['uid']]=[
						'uid'=>$user_info['uid'],
			...
			case 'add_published_user':
				...
				if($_POST['uid']!=$weibo_msg_published_user['uid']){
					...
					$this->model('setting')->set_vars([
						'weibo_msg_published_user'=>[
	...
	public function save_approval_item_action(){
		...
		if(!$_POST['title'] AND in_array($_POST['type'],[
				'question',
				'article',
				'received_email'
			])
		...
		if(!$_POST['content'] AND in_array($_POST['type'],[
				'answer',
				'article_comment'
			])
		){
		...
		switch($approval_item['type']){
			case 'weibo_msg':
				$this->model('openid_weibo_weibo')->update('weibo_msg',[
					'text'=>$approval_item['text']
				],'id = '.$approval_item['id']);
				break;
			case 'received_email':
				$this->model('edm')->update('received_email',[
					'subject'=>$approval_item['subject'],
					'content'=>$approval_item['content']
				],'id = '.$approval_item['id']);
				break;
			default:
				$this->model('publish')->update('approval',[
	...
	public function save_today_topics_action(){
		...
		$this->model('setting')->set_vars(['today_topics'=>$today_topics]);
	...
	public function save_receiving_email_config_action(){
		...
		if(!$_POST['protocol'] OR !in_array($_POST['protocol'],[
				'pop3',
				'imap'
			])
		){
	...
	public function save_receiving_email_global_config_action(){
		...
		$this->model('setting')->set_vars([
			'receiving_email_global_config'=>[
				'enabled'=>(in_array($_POST['enabled'],[
					'question',
					'ticket'
				]))?$_POST['enabled']:'N',
				'publish_user'=>[
